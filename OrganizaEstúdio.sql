...
CREATE TABLE IF NOT EXISTS `organizzaestudio`.`MIDIA` (
  ...
  `DataCriacao` TIMESTAMP NULL ON UPDATE CURRENT_TIMESTAMP,
  `DataAceiteMidia` DATETIME NULL DEFAULT NULL,
  `ResponsavelUpload` INT NULL,
  `StatusAceiteMidia` CHAR NOT NULL,
  PRIMARY KEY (`CdMidia`),
  INDEX `FuncioarioUpload_idx` (`ResponsavelUpload` ASC),
  CONSTRAINT `ResponsavelUpload`
    FOREIGN KEY (`ResponsavelUpload`)
    REFERENCES `organizzaestudio`.`USUARIO` (`CdUsuario`)
    ON DELETE CASCADE
    ON UPDATE CASCADE)
ENGINE = InnoDB
